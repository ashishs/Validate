...
namespace Validate.ValidationExpressions
{
    public class ContainsTargetMemberExpression<T, U, V> : TargetMemberValidationExpression<T, U> where U : IEnumerable<V>
    {
        ...
        public override ValidationMethod<T> GetValidationMethod()
        {
            ...
            var validationMessage = message.Populate(targetType: GetTargetTypeName(), targetMember: GetTargetMemberName(), targetValueContains: containsDisplay);
            ...
            Func<Validator<T>, Validator<T>> validation = (v) =>
                                                              {
                                                                  ...
                                                                  if (valuesNotContained.Count() > 0)
                                                                      v.AddError(new ValidationError(validationMessage.Populate(targetValue: target).ToString(), target,
                                                                                 cause: "{{ The target member {0}.{1} with value {2} did not contain value(s) {{ {3} }} }}".WithFormat(GetTargetTypeName(), GetTargetMemberName(), target, valuesNotContained.Select(val => val.ToString()).Join(" | "))));
                                                                  return v;
                                                              };
